...
def verificar_e_obter_confirmacao(caminho_arquivo):
    if os.path.exists(caminho_arquivo):
        return False
    resposta = input(f"O arquivo {caminho_arquivo} não existe em formato TXT. Deseja renomear o arquivo XLS para TXT? (S/N):")
...
if os.path.exists(pasta_origem):
    if os.path.exists(caminho_arquivo_txt):
        ...
        #Verifique-se há um arquivo XLS com o mesmo nome
        caminho_arquivo_xls = caminho_arquivo_txt.replace('.txt','.xls')
        if os.path.exists(caminho_arquivo_xls):
        #Verifique se o usuário deseja renomear o arquivo XLS para TXT
            confirmacao = verificar_e_obter_confirmacao(caminho_arquivo_txt)

            if confirmacao:
                #Copie o arquivo XLS e renomeie-o para TXT
                shutil.copy2(caminho_arquivo_xls, os.path.join(pasta_destino, nome_arquivo))
                print(f'Arquivo {caminho_arquivo_xls} copiado e renomeado para {nome_arquivo} em {pasta_destino}')

        else:
            print(f'Arquivo {nome_arquivo} em formato TXT não foi encontrado e não há um arquivo XLS correspondente.')
